...
use crate::plugin::{
    add_non_networked_rollback_systems, add_prediction_diff_systems, add_prediction_systems,
...
use bevy_replicon::shared::replication::diff::{DiffWire, Diffable as RepliconDiffable};
...
impl<C> PredictionRegistrationExt<C> for ComponentRegistration<'_, C> {
        ...
        C: SyncComponent + RepliconDiffable,
    {
        ...
        registry.register::<C>(prediction_history_id, confirmed_history_id);
        add_prediction_systems::<C>(self.app);
        add_prediction_diff_systems::<C>(self.app);
...
fn write_history_diff<C: SyncComponent + RepliconDiffable>(
    ...
    let mut receiver = entity
        ...
        .cloned()
        .unwrap_or_default();
    if let DiffWire::Snapshot { cursor, mut value } = wire {
...
fn diff_wire_and_tick<C: SyncComponent + RepliconDiffable>(
    ...
    entity: &mut DeferredEntity,
    message: &mut Bytes,
) -> Result<Option<(Tick, DiffWire<C, C::Patch>)>> {
...
mod tests {
    ...
    #[derive(Component, Clone, PartialEq, Debug, serde::Deserialize, serde::Serialize)]
    struct DiffBuilderComponent(u32);

    impl RepliconDiffable for DiffBuilderComponent {
        type Patch = u32;
    ...
    #[test]
    fn predict_diff_builder_enables_prediction_for_diff_replication() {
        let mut app = prediction_app();

        app.component::<DiffBuilderComponent>()
            .replicate_diff()
            .predict_diff()
            .add_custom_interpolation_diff();

        let registry = app.world().resource::<PredictionRegistry>();
        assert!(registry.predicted::<DiffBuilderComponent>());

        let interpolation_registry = app.world().resource::<InterpolationRegistry>();
        assert!(interpolation_registry.interpolated::<DiffBuilderComponent>());
    }
