...
impl<C> ComponentRegistration<'_, C> {
    ...
    /// Register this component with Replicon's patch-based diff replication.
    ///
    /// Mutations must be recorded through Replicon's diff API so patches can be
    /// generated.
    ...
    /// Register this component with Replicon's patch-based diff replication and
    /// an archetype filter.
    pub fn replicate_diff_filtered<F: FilterRules>(self) -> Self
    where
        C: RepliconDiffable,
    {
        self.app.replicate_diff_filtered::<C, F>();
        self
    }
